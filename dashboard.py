...
def create_weathersit_visualization(data):
    plt.figure(figsize=(8, 5))
    sns.barplot(x="cnt", y="weathersit_label", data=data, palette="coolwarm")
...
season_data, day_df = analyze_season_data()

seasons = season_data["season_label"].unique()

selected_season = st.sidebar.selectbox("Pilih Musim", ["Semua"] + list(seasons))

# Filter data berdasarkan musim yang dipilih
if selected_season != "Semua":
    filtered_data = day_df[day_df["season_label"] == selected_season]
else:
    filtered_data = day_df

# Menampilkan visualisasi berdasarkan filter
st.subheader("Visualisasi Rata-rata Penyewaan Sepeda Berdasarkan Musim")
if selected_season != "Semua":
    st.write(f"Menampilkan data untuk musim: **{selected_season}**")
else:
    st.write("Menampilkan data untuk semua musim.")

create_season_visualization(season_data[season_data["season_label"] == selected_season] if selected_season != "Semua" else season_data)

# Isi konten tampilan dashboard
